	...
	public function pricing_table_sc( $atts, $content = null ) {
		...
		foreach ( (array)$pricing_table['col-data'] as $col_index => $col_data ) {
			...
			if ( !empty( $column_style_data['type'] ) ) {
				...
				$price_format = !empty( $general_settings['currency'][0]['position'] ) && $general_settings['currency'][0]['position'] == 'left' ? '<span data-id="price"><span data-id="amount">%2$s</span><span data-id="currency">%1$s</span></span>' : '<span data-id="price"><span data-id="amount">%2$s</span><span data-id="currency">%1$s</span></span>';
		...
		/* Import icon fonts */
		if ( preg_match( '/<i[^<>]*class=(\"|\')(.*)?fa(.*)?(\2)/', $html ) ) $font_css .= sprintf( '@import url(%s);', $this->plugin_url . 'assets/lib/font_awesome/css/font-awesome.min.css' ); 
		if ( preg_match( '/<i[^<>]*class=(\"|\')(.*)?linecon(.*)?(\2)/', $html ) ) $font_css .= sprintf( '@import url(%s);', $this->plugin_url . 'assets/lib/linecon/linecon.min.css' );		
		if ( preg_match( '/<i[^<>]*class=(\"|\')(.*)?icomoon(.*)?(\2)/', $html ) ) $font_css .= sprintf( '@import url(%s);', $this->plugin_url . 'assets/lib/icomoon/icomoon.min.css' );
