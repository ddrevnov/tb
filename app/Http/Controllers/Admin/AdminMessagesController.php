...
{
    //страница с чатом и всеми возможными участниками
    public function messages(Employee $employee, DirectorEmployee $directorEmployee)
    {
        ...
        $this->data['admin_employees'] = $employee->getEmployeeNewMessage($this->idAdmin);

        return view('admin.messages', $this->data);
    }
    ...
    //ajax запрос сообщений при открытии чата с другим пользователем. повторное обращение возвращает еще 10 сообщений
    ...
    //ajax запрос на всех страницах новых сообщений для вывода возле конвертика
    public function getNewMessages()
    {
        ...
        $who[2] = DirectorEmployee::whereIn('director_employees.user_id', $fromIds)
                                    ...
                                    ->get()->toArray();
        //приведение данных в нужный для фронта вид
        $i = 0;
        foreach($who as $w){
            foreach ($w as $n){
                $result[$i] = $n;
